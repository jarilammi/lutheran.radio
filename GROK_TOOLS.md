...
- Live Activity language / lock-stretch work: read `docs/Live-Activity-Stacking-and-Media-Surfaces.md` and `docs/Widget-Functionality-Roadmap.md` (ContentState convergence) before changing ensure/recreation policy. `Activity.request` visibility is start/recreate only; stall truth is `contentUpdates` / delayed re-read. Never end the only interactive LA while ``isInteractiveLiveActivityRequestEligible`` is false (language or visual lag); owned surfaces update only — never ``startActivity()`` end+request. Never invent `.playing` during stream-switch hold; presentable switch stays Connecting via ``resolveContentPushVisual``. Dual-axis `.playing` after hold clear stays; ineligible dest-language is language-only. Soft ensure may lag under continuous lock; become-active heals. Product comments must use mechanism names only (never living prompts or evidence filenames).
