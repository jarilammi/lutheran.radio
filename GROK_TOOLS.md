...
- Live Activity language / lock-stretch work: read `docs/Live-Activity-Stacking-and-Media-Surfaces.md` and `docs/Widget-Functionality-Roadmap.md` (ContentState convergence) before changing ensure/recreation policy. `Activity.request` visibility is start/recreate only; stall truth is `contentUpdates` / delayed re-read. Owned interactive surfaces update only — never ``startActivity()`` end+request. Soft language ensure may lag under continuous lock-ineligible; become-active heals; never end the only interactive LA while request is ineligible solely for language lag. Product comments must use mechanism names only (never living prompts or evidence filenames).
