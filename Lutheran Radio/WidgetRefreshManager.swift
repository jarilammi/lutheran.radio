...
//   callback). Concurrent identical sticky-pause **or** ``.playing`` wakes also coalesce
//   against ``inFlightIdenticalWake`` (visual + language + error + paint epoch) because
//   ``lastKnownState`` is booked only after a real wake — otherwise dual-path settle
...
//   LastKnown identity skip stays non-playing so later ``.metadataDidUpdate`` can still
//   adaptive-debounce a program-title wake after the first playing execute.
...
final class WidgetRefreshManager: @unchecked Sendable {
    ...
    /// In-flight identical-wake identity (visual + language + error + paint epoch).
    ///
    /// Set synchronously when a sticky-pause or ``.playing`` ``reloadTimelines`` is scheduled;
    /// cleared when ``performRefreshIfNotStale(for:)`` returns (execute or discard). Lets later
    /// identical ``refreshIfNeeded`` calls coalesce without booking ``lastKnownState`` before
    ...
    ///
    /// Connecting stays off this token (deferral / lastKnown identity skip). LastKnown
    /// identity skip still does not apply to ``.playing`` (later metadata wakes).
    /// - SeeAlso: ``shouldCoalesceInFlightIdenticalRefresh(requestedVisual:requestedLanguage:requestedHasError:requestedPaintEpoch:inFlight:)``.
    var inFlightIdenticalWake: InFlightIdenticalWake?
    private var inFlightIdenticalWakeGeneration: UInt64 = 0
    ...
    ///   ``shouldCoalesceInFlightIdenticalRefresh(requestedVisual:requestedLanguage:requestedHasError:requestedPaintEpoch:inFlight:)``,
    ///   docs/Event-Driven-Refactor-Roadmap.md (dual-path inventory),
    ///   docs/Widget-Functionality-Roadmap.md (refresh inventory).
    func refreshIfNeeded(
        ...
        // Concurrent sticky-pause or playing bursts (event path + teardown dual-path, often
        // after ``liveChromeIdentitySkipWake``) schedule before ``lastKnownState`` is booked.
        ...
        // calls until the first wake returns. Language changes still execute (token includes
        // language). Connecting stays on deferral / lastKnown identity skip — not this token.
        // LastKnown identity skip still excludes ``.playing`` so later metadata can debounce.
        let paintEpochNow = SharedPlayerManager.loadHomeWidgetInteractivePaintEpoch()
        if Self.shouldCoalesceInFlightIdenticalRefresh(
            ...
            inFlight: inFlightIdenticalWake
        ) {
            #if DEBUG
            ...
            recordDebounceOutcome(
                Self.isStickyPauseKind(newState.visualState)
                    ? .coalescedInFlightIdenticalPause
                    : .coalescedInFlightIdenticalPlaying
            )
            #endif
            return
        }
        ...
        // The first such exception still executes once; later identical pause-kind or
        // playing calls while that wake is in flight coalesce via ``inFlightIdenticalWake``.
    ...
    static func shouldCoalesceIdenticalNonPlayingRefresh(
        ...
        case .prePlay, .cleared, .userPaused, .thermalPaused, .securityLocked:
            return true
        case .playing:
            // LastKnown identity skip does not apply to playing: later
            // ``PlayerEvent/metadataDidUpdate`` must still adaptive-debounce a program-title
            // wake after the first playing execute books lastKnown. Concurrent settle
            // storms coalesce against ``shouldCoalesceInFlightIdenticalRefresh`` instead.
            return false
        @unknown default:
            return false
        }
    ...
    ///
    /// LastKnown identity skip stays on this set plus Connecting. In-flight identical-wake
    /// coalesce is the broader ``isInFlightIdenticalWakeKind`` (this set **plus** ``.playing``).
    ///
    /// - Parameter visual: Candidate ``PlayerVisualState``.
    /// - Returns: `true` for ``.userPaused``, ``.thermalPaused``, and ``.securityLocked``.
    /// - SeeAlso: ``isInFlightIdenticalWakeKind(_:)``,
    ...
    /// Whether `visual` participates in in-flight identical-wake coalesce.
    ///
    /// Sticky pause/lock and active ``.playing`` share one token because dual-path settle
    /// bursts schedule before ``lastKnownState`` is booked. Connecting stays on deferral /
    /// lastKnown identity skip. ``.cleared`` stays urgent (privacy / factory).
    ///
    /// LastKnown identity skip still does **not** apply to ``.playing``: later
    /// ``PlayerEvent/metadataDidUpdate`` must remain eligible for adaptive debounce after
    /// the first playing wake books. This helper is schedule-time in-flight only.
    ///
    /// - Parameter visual: Candidate ``PlayerVisualState``.
    /// - Returns: `true` for sticky pause/lock and ``.playing``.
    /// - SeeAlso: ``shouldCoalesceInFlightIdenticalRefresh(requestedVisual:requestedLanguage:requestedHasError:requestedPaintEpoch:inFlight:)``,
    ///   ``isStickyPauseKind(_:)``.
    static func isInFlightIdenticalWakeKind(_ visual: PlayerVisualState) -> Bool {
        switch visual {
        case .userPaused, .thermalPaused, .securityLocked, .playing:
            return true
    ...
    /// Pure policy: whether a sticky-pause or ``.playing`` ``refreshIfNeeded`` should no-op
    /// because an identical wake is already in flight.
    ///
    /// ``lastKnownState`` is booked only after a real WidgetCenter wake. Concurrent dual-path
    /// calls (event + teardown, including the first paint-epoch exception after
    ...
    /// ``lastKnownState`` so a discarded first wake cannot strand later honest pauses or
    /// playing metadata wakes that arrive after the token clears.
    ///
    /// The **first** paint-epoch exception still executes (no in-flight token yet, or a newer
    /// epoch). Later identical calls with the **same** epoch coalesce here. Language mismatch
    /// never coalesces.
    ///
    /// - Parameters:
    ...
    static func shouldCoalesceInFlightIdenticalRefresh(
        requestedVisual: PlayerVisualState,
        ...
        guard isInFlightIdenticalWakeKind(requestedVisual) else { return false }
    ...
    /// Pause-scoped wrapper over ``shouldCoalesceInFlightIdenticalRefresh``.
    ///
    /// Existing sticky-pause policy tests call this surface. ``.playing`` never returns
    /// `true` here even when an in-flight playing token would match the general helper.
    ///
    /// - SeeAlso: ``shouldCoalesceInFlightIdenticalRefresh(requestedVisual:requestedLanguage:requestedHasError:requestedPaintEpoch:inFlight:)``.
    static func shouldCoalesceInFlightIdenticalPauseRefresh(
        ...
        return shouldCoalesceInFlightIdenticalRefresh(
            requestedVisual: requestedVisual,
            requestedLanguage: requestedLanguage,
            requestedHasError: requestedHasError,
            requestedPaintEpoch: requestedPaintEpoch,
            inFlight: inFlight
        )
    }
    ...
    /// - Important: Soft-resume non-immediate Connecting deferral,
    ///   ``shouldCoalesceIdenticalNonPlayingRefresh``, and
    ///   ``shouldCoalesceInFlightIdenticalRefresh`` are separate schedule-time policies; this
    ...
    private func scheduleCoalescedPrePlayRefresh(for state: WidgetState) {
        ...
        let workItem = DispatchWorkItem { [weak self] in
            Task { @MainActor in
                ...
                let generation = self.markInFlightIdenticalWakeIfNeeded(for: pendingState)
                await self.performRefreshIfNotStale(for: pendingState)
                self.clearInFlightIdenticalWake(generation: generation)
            }
        }
    ...
    /// Records the in-flight identical-wake token (when applicable) and schedules execute-time
    ...
    private func scheduleRefreshExecution(for state: WidgetState) {
        let generation = markInFlightIdenticalWakeIfNeeded(for: state)
        Task { @MainActor in
            await performRefreshIfNotStale(for: state)
            clearInFlightIdenticalWake(generation: generation)
        }
    }

    /// - Returns: Generation of the token when this is a sticky-pause or playing wake; `nil` otherwise.
    @discardableResult
    private func markInFlightIdenticalWakeIfNeeded(for state: WidgetState) -> UInt64? {
        guard Self.isInFlightIdenticalWakeKind(state.visualState), !state.hasError else { return nil }
        inFlightIdenticalWakeGeneration += 1
        let generation = inFlightIdenticalWakeGeneration
        inFlightIdenticalWake = InFlightIdenticalWake(
    ...
    private func clearInFlightIdenticalWake(generation: UInt64?) {
        guard let generation else { return }
        guard inFlightIdenticalWake?.generation == generation else { return }
    ...
    private func performRefresh(for state: WidgetState) async {
        ...
        // or playing bursts instead coalesce against ``inFlightIdenticalWake`` (same visual +
        // language + epoch), which is released when this method's caller finishes — including discard.
    ...
    /// after lastKnown is booked (metadata). Concurrent identical playing wakes coalesce
    /// in-flight via ``shouldCoalesceInFlightIdenticalRefresh``.
    ///
    /// - Parameters:
...
// MARK: - In-flight identical wake identity

/// Snapshot of a sticky-pause or ``.playing`` WidgetKit wake already scheduled but not yet
/// booked in ``WidgetRefreshManager/lastKnownState``.
///
...
///
/// Connecting is not this token. LastKnown identity skip still excludes ``.playing`` so
/// later ``PlayerEvent/metadataDidUpdate`` can adaptive-debounce after the first execute.
///
/// - SeeAlso: ``WidgetRefreshManager/shouldCoalesceInFlightIdenticalRefresh(requestedVisual:requestedLanguage:requestedHasError:requestedPaintEpoch:inFlight:)``,
///   ``WidgetRefreshManager/refreshIfNeeded(visualState:currentLanguage:hasError:immediate:trigger:)``,
///   docs/Home-Live-Chrome-App-Group-Mirror-Design.md (§8).
struct InFlightIdenticalWake: Equatable, Sendable {
    let visualState: PlayerVisualState
    let currentLanguage: String
    let hasError: Bool
    let paintEpoch: Int
