...
/// | instantFeedbackLanguage | String                | Play/pause: ``writeInstantFeedback(language:)`` via ``languageForInstantFeedbackWrite(_:)`` (coerces to ``settledLanguageForInstantFeedback()`` — session vs ``homeWidgetLiveChrome`` by the same freshness rule as ``resolveHomeWidgetChromeFields`` / ``languageForWidgetRefreshDerivation``; a leftover session code is not a valid match when chrome is strictly fresher); ``executeOptimisticToggle(plan:language:)`` / ``handleWidgetPlay()`` / ``handleWidgetStop()`` pass that settled code. Switch: destination after persist (destination is already the fresher settled source). | `loadSharedState` — 15 s window does **not** override ``settledLanguageForInstantFeedback()`` | Optimistic **switch** flash (destination matching the fresher settled source) or first-tap language; play/pause write follows freshness-settled chrome — does not plant a leftover session, locale, or lagging Live Activity language | Same lifetime as `isInstantFeedback` |
