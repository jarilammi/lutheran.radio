...
/// | instantFeedbackLanguage | String                | Play/pause: ``writeInstantFeedback(language:)`` via ``languageForInstantFeedbackWrite(_:)`` (coerces to settled session / ``homeWidgetLiveChrome`` when the caller disagrees); ``executeOptimisticToggle(plan:language:)`` / ``handleWidgetPlay()`` / ``handleWidgetStop()`` pass that settled code. Switch: destination after persist (destination is already settled). | `loadSharedState` — 15 s window does **not** override a disagreeing settled session or live-chrome language | Optimistic **switch** flash (destination) or first-tap language; play/pause write follows settled chrome — does not plant a locale or lagging Live Activity language | Same lifetime as `isInstantFeedback` |
