...
/// | pendingAction           | String ("play","pause","switch") | Extension-hosted widget / Control / LA intents (`signalWidgetPendingAction` / ``signalWidgetSwitchAction``). **Not** presentable in-process ``LiveActivityIntent`` / ``AudioPlaybackIntent`` chips (``WidgetIntentExecution/executeInProcessStreamSwitch``) | ``getPendingActionIfFresh(maxAge:)`` (SceneDelegate / Darwin / launch burst → coordinator drain); raw ``getPendingAction()`` for presence only | One-shot command from extension process to main app | Cleared by ``clearPendingAction(actionId:)``, ``clearPendingActionMailbox()``, cold-launch residual discard, privacy clear; **not** executed until main mailbox armed this process |
