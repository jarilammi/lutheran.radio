...
/// | instantFeedbackLanguage | String                | Play/pause: session / ``homeWidgetLiveChrome`` via ``languageForLiveActivityOrWidgetOptimistic()`` / ``executeOptimisticToggle(plan:language:)``; switch: destination after persist | `loadSharedState` — 15 s window does **not** override a disagreeing settled session or live-chrome language | Optimistic **switch** flash (destination) or first-tap language; play/pause must not re-introduce a prior Live Activity language | Same lifetime as `isInstantFeedback` |
