...
- **Test documentation**: Tests should state the specific invariant, permanent/transient error case, or behavioral property they protect (see existing Core test patterns). For tests involving AsyncStreams, Live Activities, or widget state, also follow the fast/reliable patterns and "test execution patience" guidance in the "Test Execution Patience and Fast, Reliable Test Patterns" section below. Reference the canonical examples in `SharedPlayerManagerEventTests.swift` (emission/replay) and sibling suites for media-surface / cold-launch / latency coverage.
...
- `WidgetInteractiveIntents.swift` — Live Activity / home / Control interactive AppIntent **types** (`AudioPlaybackIntent` / `LiveActivityIntent` on the main-app profile; `supportedModes = .background`)
...
- `Lutheran RadioTests/SharedPlayerManagerEventTests.swift` — primary *usage* suite for core emission/replay: `testMakeEventsStreamWithReplayYieldsCurrentStateThenLiveEvents` (Tier 3 prefix), `testLiveEmitsTransitionEventsForStopPauseFailAndIntent` (live coverage), `testReplayStreamPrefixesStateThenForwardsLiveStopEmissionsInOrder` (hybrid replay + emission order); setUp via ``prepareSharedPlayerManagerEventTestIsolation``
...
- `Lutheran RadioTests/DirectStreamingPlayerEngineTests.swift` — real-engine attach/recovery isolation, UITestMode audio short-circuits, and ``shouldSkipForceWidgetSaveOnStableStatus`` policy
...
Following these keeps the full test suite fast and repeatable while still giving meaningful coverage of the event-driven architecture and media surfaces. Before writing new tests in this area, re-read `Lutheran RadioTests/Support/PlayerEventTestSupport.swift` (canonical collectors + suite isolation helpers) and the core methods under "Replay Scenario", "Live Emission Coverage", and "Replay Forwarding & Emission Order" in `SharedPlayerManagerEventTests.swift`. Place failure-classification, mutation-order, stream-switch, media-surface, cold-launch, and latency cases in the matching sibling suite rather than re-growing the core emission file.
...
| `Core/Configuration/SecurityConfiguration.swift`  | Centralized security policy: expected model, sole live leaf via `pinnedLeafFingerprintDigest` / `pinnedSiikkariLeafFingerprintDigest` / `pinnedFingerprintDigests`, `preferredStreamingDomainSuffixes` (`siikkari.net`), `securityModelDomains`, transition dates, `modelCacheDuration` (DNS 1h), `certificateValidationCacheDuration` (cert 10m) | Authoritative digests; colon-hex views are derived; media apex and TXT hosts are independent SSOTs; model vs cert caches are distinct; do not keep retired leaves on the acceptance list |
...
| `LutheranRadioWidget/`                            | Home-screen / Control / LA SwiftUI shells                         | Thin delegates; presentation via `import WidgetSurface`; interactive AppIntent types live in membership-exception `WidgetInteractiveIntents.swift`; same `Core` security rules |
| `RadioLiveActivityManager.swift`                  | Live Activity start/update, language + playing soft ensure, deferred recreation while request-ineligible, foreground owned-surface heal | Never end the only interactive Live Activity while ``isInteractiveLiveActivityRequestEligible`` is false (language or visual lag). ``startActivity()`` is request-eligibility gated: owned surfaces update only — never `Activity.request` / leading end while ineligible. Concurrent ``startActivity()`` / `.startOrUpdate` joins one in-flight `Activity.request` (``interactiveLiveActivityStartDisposition`` ``.joinInFlightRequest``) — never two interactive ids; after success, sibling reap with ``.immediate``. Never invent `.playing` during stream-switch hold; presentable switch stays Connecting via ``resolveContentPushVisual``. Dual-axis `.playing` after hold clear stays; ineligible dest-language is language-only. Missing-card ensure uses ``sessionNeedsInteractiveLiveActivity`` — factory-idle `.prePlay` (pipeline not started, hold inactive) does not request; Connecting attach / sticky pause still do. Stall truth is `contentUpdates` / delayed re-read, not one post-await read. Language-ensure attempts consume only on committed observation (`contentUpdates` that still lags, or delayed re-read whose language moved off the pre-push value); a delayed re-read still on the pre-push language does not consume or quiet. Skip the next soft push while ``inFlightContentPushCandidate`` is unconfirmed (max 3; 200/400/800 ms ineligible spacing; never end while ineligible). Soft ensure may lag under continuous lock; become-active heals. Canonical: `docs/Live-Activity-Stacking-and-Media-Surfaces.md`. |
...
| `SharedPlayerManager.swift` (+ extensions) + `DirectStreamingPlayer+WidgetStub.swift` + `SecurityValidationFacade.swift` + `WidgetDisplayModels.swift` + `WidgetIntentExecution.swift` + `WidgetInteractiveIntents.swift` + `WidgetRefreshManager.swift` (+ test support) + `MediaTransportLatencyTimeline.swift` | Membership-exception SSOT: actor state, named security call-site intents, intent execution + interactive AppIntent types (`AudioPlaybackIntent` / `LiveActivityIntent`), snapshot hygiene, widget refresh; DEBUG transport latency timeline | Compiled into app + extension + `LutheranRadioWidgetTests`. Pure presentation lives in `WidgetSurface/`. Never duplicate widget state logic. Security *policy* stays in `Core/`. |
...
- `Core/Configuration/` — `SecurityConfiguration.swift` (constants, policy, transition dates, sole live leaf digests via `pinnedLeafFingerprintDigest` / `pinnedSiikkariLeafFingerprintDigest` / `pinnedFingerprintDigests`, `preferredStreamingDomainSuffixes`, `securityModelDomains`, expected model). Never duplicate these values elsewhere.
