...
- `RadioPlayerCoordinator+StreamSwitch.swift` — paused/blocked switch tails re-sample SSOT via ``paintBlockedSwitchChromeFromSSOTIfIntentStillNotActive()`` (do not overwrite Connecting after a later play); in-process chip language chrome via ``syncLanguageChromeFromChosenStream(_:isAttaching:)``.
