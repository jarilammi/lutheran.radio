...
2. **Inside `RadioLiveActivityManager`**:
   ...
   - After a bounded streak of `Activity.update` results that leave system-held chrome lagging (system `content.state` language still prior, or visual stuck on `.userPaused` while candidate needs Connecting/playing), recreation is considered. End + ``startActivity()`` runs **only when** interactive request is eligible (activities enabled + application active); otherwise the existing surface is kept and a pending ensure is recorded for the next foreground cycle. See docs/Live-Activity-Stacking-and-Media-Surfaces.md.
