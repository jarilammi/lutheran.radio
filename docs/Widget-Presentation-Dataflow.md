...
- **On observed termination** (AppDelegate `applicationWillTerminate`, SceneDelegate `sceneDidDisconnect`, `UIApplication.willTerminateNotification`):
  - Delivered quit may invoke both `applicationWillTerminate` and `sceneDidDisconnect`. ``performSessionTeardownSynchronouslyForTermination()`` is **single-flight**: the first call claims ``hasCompletedProcessExitSessionTeardown()``, writes the liveness sentinel, and waits on Live Activity end; the second is a no-op. Both call sites remain (multi-window / some kills deliver disconnect without `applicationWillTerminate`). Force-quit and reboot often deliver neither. After the latch is claimed, this process must not restore `lastUpdateTime` or restamp ``homeWidgetLiveChrome``: swipe-up Home can still run ``sceneDidEnterBackground`` (`recordWidgetLiveness` / ``saveCurrentState()``, including identical-snapshot skip which still bumps) while disconnect teardown has already written sentinel `0` and cleared live chrome. A post-sentinel bump plus factory ``.prePlay`` paints interactive Connecting instead of `tap_to_open`. ``isMainAppProcessRecentlyActive()`` re-syncs the App Group suite on read so Providers see sentinel `0` immediately.
  - **On resign-active (lock / inactive):** ``resignActiveSessionTeardownDecision()`` skips session teardown when ``shouldPreserveSessionAcrossResignActive`` (audible `.playing` **or** claimed Connecting via stream-switch hold / start pipeline) and when visual is already `.cleared` (``clearAllLocalState()`` already ended the Live Activity). Factory-idle `.prePlay` (no hold, no pipeline) and sticky `.userPaused` still run ``performSessionAndWidgetTeardown``. Sleep-timer elapsed (grey `.userPaused` + `.sleepTimer` intent) is not a claimed attach: resign-active still tears down. Elapsed pause itself awaits ``DirectStreamingPlayer/stopAndWait(reason:silent:applyUserPauseVisualLock:)`` (`.interruption`) then ``deactivateAudioSessionAsync()`` so the hard-stopped engine does not keep an audio session; it does **not** run ``performSessionAndWidgetTeardown`` (paused Live Activity stays until resign-active / terminate). Resume remains ``setPlaying()``.
  ...
  - While the main process is alive, ``WidgetRefreshManager`` also identity-coalesces identical connecting / sticky chrome (`shouldCoalesceIdenticalNonPlayingRefresh`) so attach-path and dual-path storms do not spam `reloadTimelines` for unchanged language/visual. Concurrent identical sticky-pause **or** ``.playing`` wakes coalesce against ``shouldCoalesceInFlightIdenticalRefresh`` (visual + language + error + paint epoch) because ``lastKnownState`` is booked only after a real wake — the first paint-epoch exception after ``liveChromeIdentitySkipWake`` still executes once. LastKnown identity skip still excludes ``.playing`` so later ``PlayerEvent/metadataDidUpdate`` can adaptive-debounce a program-title wake after the first playing execute. Language changes always reload (and mark the candidate **immediate** for execute-time wake discard). Scheduler labels are not rendered home chrome. Execute-time home **wake** discard is unified in ``refreshWouldDiscardHomeWake(executing:memory:session:isImmediate:)`` (memory lag first via ``refreshWouldRegressMemoryAuthority``, then session lag via ``refreshWouldRegressPersistedSnapshot``). Reload is wake-only; Providers paint session + ``homeWidgetLiveChrome``. Discards cover residual sticky after intentional Connecting, premature ``.playing`` during switch/connect hold, non-immediate post-audible Connecting, and soft-resume reverse-race pause (session leg). See docs/Home-Live-Chrome-App-Group-Mirror-Design.md §8.
  ...
  - **Home stream-switch first-paint honesty:** On active-play home (and extension LA co-path) stream switch, the optimistic session snapshot and first ``refreshIfNeeded`` target use ``WidgetIntentCoordinators/optimisticLiveActivityVisualForStreamSwitch(from:)`` — actively playing → Connecting (``.prePlay``); sticky pause preserved. Destination language advances immediately without claiming mid-switch ``.playing`` during silent attach hold. Authoritative ``.playing`` arrives later via main-app attach / ``setPlaying()``. A new stream choice clears ``isPlaybackStartPipelineActive`` in ``resetToPrePlayForNewStream`` (and the in-app skip-reset play continuation) so the follow-on ``play()`` attaches the destination instead of skipping as a duplicate of the superseded Connecting start. Same-stream Connecting still no-ops a second ``userRequestedPlay()``. Do not arm the latch before ``play()``. At execute time, memory Connecting **discards premature ``.playing``** so a lagging dual-path playing target cannot flash mid-hold. Immediate language-change Connecting may advance a lagging session snapshot still on prior-language ``.playing``. Soft-resume prePlay→playing coalesce, sticky-pause directionality, and privacy write suppression are unchanged. Wired from widget-path ``switchToStream`` snapshot write and ``WidgetIntentExecution/executeHomeWidgetStreamSwitch(languageCode:)``.
  - **Sticky pause home wake discard:** On ``stop()`` / sticky user-pause lock, when the home-widget privacy gate allows writes, ``SharedPlayerManager`` writes an early sticky ``.userPaused`` session snapshot (`persistEarlyStickyUserPausedSnapshotIfPrivacyAllows`) **before** Icecast hard teardown and the deferred authoritative ``saveCurrentState()``. That keeps non-carried events (`streamDidStop`, `playbackIntentChanged`, `streamDidPause`) and the session lag leg of ``refreshWouldDiscardHomeWake`` from re-reading lagging ``.playing``. Separately, the session leg is **directional**: non-immediate / adaptive-debounced ``.userPaused`` still discards when the session snapshot has already advanced to ``.playing`` (play-after-pause reverse race), but **immediate** sticky-pause / teardown urgency does **not** discard ``.userPaused`` solely because the snapshot still lags on ``.playing`` (forward stop). Privacy write suppression with zero widgets is unchanged (early sticky is a no-op when the gate is closed). User pause nils the live item; home Play after pause is Connecting → ``setPlaying``, not same-stream gapless resume.
...
| **User-initiated main open** | Human started main (icon, Siri, `lutheranradio://open`, other open-only launch) **and** a scene became presentable | After factory reset, when this-process sticky pause is absent: special tuning clip, then ``SharedPlayerManager/play()`` (security still runs before attach). Product language: **open app = radio** on the first become-active — not a background scene-create after jetsam | `ViewController` factory-hygiene Task, ``resetToFactoryDefaultsOnLaunch()``, ``RadioPlayerCoordinator/markPresentableColdLaunchPlaybackReady(initialStream:)``, ``RadioPlayerCoordinator/notePresentableSceneForColdLaunchPlayback()``, ``playSpecialTuningSound``, cold-play path; **not** prior-process App Group play restore (OI-1) |
...
| **Presentable in-process chips** | User tapped a lock-screen Live Activity or home-widget language chip while the main app hosts `LiveActivityIntent` / `AudioPlaybackIntent` | Engine switch + in-app flags / needle / background; **no** `pendingAction*` / Darwin ``radio.lutheran.widget.action`` note | ``WidgetIntentExecution/executeInProcessStreamSwitch`` + ``RadioPlayerCoordinator/syncLanguageChromeFromChosenStream``. Do **not** route through ``handleWidgetSwitchToLanguage`` just to paint flags. Session language snapshot and Live Activity language mirror still write. |

#### User-initiated main open (intentional cold play)

Opening the **main** process after a cold start is expected to start radio (special tuning, then stream) when sticky intent does not block **and** a scene is presentable (`sceneDidBecomeActive`, or already `.active` after factory hygiene). That is a **user-driven** product choice: the human brought the app to the foreground. Factory reset, mailbox arm, and `.prePlay` UI still run immediately even if iOS created the process in the background (jetsam / last-media relaunch); auto-play waits. Headset remotes are installed at process start without calling ``play()``. Engine construction does **not** activate `AVAudioSession`. Factory-reset teardown still *enqueues* session deactivate (Now Playing phase 2, detached so factory hygiene can return within MediaRemoteUI’s launch time budget). First special-tuning clip and ``play()`` await ``DirectStreamingPlayer/configureAudioSessionAsync()``, which waits for that mutation on ``audioSessionMutationTail`` before `setCategory` / `setActive`. Overlapping `setCategory` with an in-flight SessionCore deactivate returns OSStatus -50. SessionCore deactivate of a session this process never configured (category not `.playback`) is skipped (``DirectStreamingPlayer/shouldSkipSessionCoreDeactivate``): that completed SessionCore call still leaves the next `setCategory(.playback)` returning OSStatus -50. That skip is instant, so first presentable configure still settles SessionCore before `setCategory` (``DirectStreamingPlayer/shouldSettleSessionCoreBeforeFirstPlaybackCategory``). Privacy / stop / sleep-timer still deactivate after this process has applied `.playback`. Does **not** restore init-time ``setupAudioSession()``.
...
- **Not** performed by the widget extension; hearing special tuning proves the **main** presentable cold-launch path ran after a user (or system open-URL) brought a scene to the foreground.
- **Not** a background scene-create after jetsam or last-media relaunch — that process waits for the next presentable open.
...
| App relaunch / main-app play   | In-process visual + `PlaybackIntent` after ``resetToFactoryDefaultsOnLaunch()`` | Factory reset + sticky intent SSOT; **user-initiated main open** on the first presentable scene (special tuning then ``play()`` when allowed); **not** termination sentinel; **not** prior-process disk play restore; residual reboot chrome while main is not resident is a separate honesty path | Factory-hygiene Task + presentable cold launch / `play()` / this-process resurrection | No prior-process play gate; open = radio when sticky pause absent **and** the scene is presentable |
...
2. **Inside `RadioLiveActivityManager`**:
   ...
   - ``ensureAuthoritativeLanguageContentIfNeeded()`` re-pushes (up to ``authoritativeLanguageContentEnsureMaxAttempts`` **committed** observations) when destination language from ``liveActivityLanguageCodeForContentPush()`` still differs from owned / last language (peer to playing ensure). Immediate post-await `content.state` does not consume an attempt; skip the next `Activity.update` while ``inFlightContentPushCandidate`` is unconfirmed. A delayed re-read that still shows the pre-push language is uncommitted apply (``shouldTreatLanguageEnsureObservationAsUncommittedApply``) and does not consume or quiet; prefer `contentUpdates` as mismatch truth. After committed budget exhaustion while interactive request is ineligible, language ensure quiet pending stops status-driven thrash for that destination until destination change, eligibility, become-active, `contentUpdates`, or explicit pause dest-language clear via ``shouldClearLanguageEnsureQuietPendingOnExplicitPause`` when dest lags owned. Settle / long-horizon / dual-axis follow-up record language quiet only through ``quietPendingDestinationAfterLanguageEnsureExhaustion``. Language-only ``updateCurrentActivity`` re-pushes defer while quiet; visual mutations still push. Freeze-held pause (owned visual already ``.userPaused``) is language-only, so quiet would defer dest-language — ``recordOptimisticToggleContent(.userPaused)`` clears quiet, cancels language long-horizon and post-settled retries, and stamps ``explicitPauseNeedsDestinationLanguageUpdate`` so engine-complete dest-language `Activity.update` is not quiet-deferred after an uncommitted apply (Live Activity App Intent Darwin pause and Now Playing pause/stop both ``stop()``). Playing toggle does **not** clear language quiet. Also on foreground / become-active via ``ensureAuthoritativeContentOnForegroundIfNeeded()`` when ownership is non-nil (clears language + playing quiet first; dual SceneDelegate hooks debounced by ``shouldInvokeOwnedSurfaceForegroundEnsure`` while still consuming quiet/pending).
...
3. **Lock-screen toggle optimistic ContentState** (intent path, main or extension host):
   ...
   - That helper updates interactive `Activity` instances with ``ContentState/replacingVisualState(_:)`` (program metadata **and** `currentLanguage` preserved) and, on the main app, ``RadioLiveActivityManager/recordOptimisticToggleContent(visualState:)`` so ``lastPushedContent`` matches the optimistic glyph before engine-complete refresh. Explicit pause also clears language-ensure quiet when dest language still lags owned, cancels language long-horizon / post-settled retries, and stamps ``explicitPauseNeedsDestinationLanguageUpdate`` so that refresh dest-language `Activity.update` is not quiet-deferred over freeze-held ``.userPaused`` after an uncommitted apply.
   - ``SharedPlayerManager/stop()`` also warms the durable mirror at sticky lock and (main app) pushes optimistic ``.userPaused`` ContentState before Icecast hard teardown — pause honesty does not require owned visual to have been ``.playing`` first.
...
### In-process Live Activity / home chip language chrome

When the main app hosts ``LiveActivitySwitchStreamIntent`` / ``SwitchStreamIntent`` (`LiveActivityIntent` / `AudioPlaybackIntent`), ``WidgetIntentExecution/executeInProcessStreamSwitch(targetStream:)`` switches the engine **without** writing `pendingAction*` + Darwin ``radio.lutheran.widget.action``. Chips already run in the app; they must not leave a disk “drain later” note. That path is a **language-chrome owner** alongside Darwin ``switchToStreamFromWidget`` (extension / cold host only): after engine model + destination stamp it calls ``RadioPlayerCoordinator/syncLanguageChromeFromChosenStream(_:isAttaching:)`` (flag `selectedStreamIndex`, `PlayerViewModel` needle, decorative background, awaited ``updateUserDefaultsLanguage``). Connecting attach defers the background (`scheduleDeferredForStreamSwitch`); sticky pause applies immediately (`update(for:)`). Idempotent if the same language is already selected or if Darwin drain already painted it. Do **not** write Darwin from the in-process host. Do **not** call ``completeStreamSwitch`` (tuning). Do **not** route chrome through ``handleWidgetSwitchToLanguage`` just to move flags. The widget extension still writes pending + Darwin via ``signalWidgetSwitchAction``. Gate: `testInProcessStreamSwitchSyncsMainAppLanguageChromeWithoutPendingDarwin`, `testInProcessStreamSwitchWhileAttachingSyncsMainAppLanguageChromeWithoutPendingDarwin`, `testInProcessHomeWidgetStreamSwitchSyncsMainAppLanguageChromeWithoutPendingDarwin`, `testWidgetSwitchPreservesUserPausedVisualInOptimisticSnapshot`, `testSignalWidgetSwitchActionWritesPendingSwitchAndLanguage`.

Paused/blocked stream-switch tails (``switchToStreamFromWidget``, ``completeStreamSwitch``) must not hard-code ``updateUI(for: .userPaused)`` after awaits. A later explicit play can already have painted Connecting (``.prePlay``) via SSOT observation while the blocked path is still suspended. After those awaits, ``paintBlockedSwitchChromeFromSSOTIfIntentStillNotActive()`` re-samples ``currentPlaybackIntent`` / ``currentVisualState``: if intent is still not active, paint current visual SSOT (typically sticky pause; never overwrite ``.securityLocked`` / ``.cleared`` with hardcoded grey); if intent is already active, leave chrome to SSOT observation / the play path. The blocked branch never auto-resumes.

### In-app privacy-clear confirmation alert

The sleep-timer surface in `PlaybackControlsView` is a SwiftUI `.confirmationDialog`. The destructive “Clear local state” row **arms** ``SleepTimerPrivacyClearPresentation``; it does not invoke `onClearLocalStateTapped` while the dialog is still `isPresented`. After `isPresented` is false, ``RadioPlayerCoordinator/confirmAndClearLocalState()`` builds the secondary `UIAlertController`. ``ViewController/presentCoordinatorAlertAfterOutgoingPresentationSettles(_:)`` presents that alert only when ``CoordinatorAlertPresentationSettle/canPresentCoordinatorAlert(outgoingPresentedViewController:sceneContainsGlassPopoverHost:)`` is true: hosting-controller presented chain empty **and** no leftover `GlassPopoverContentViewRepresentable` host in the window scene (then one extra run-loop hop and `layoutIfNeeded`). `presentedViewController == nil` is not sufficient on iOS 26+ — glass can remain at autoresizing width 320 after the presented chain clears, fighting `_UIAlertControllerPhoneTVMacView` content width ~357. UIKit recovers by breaking the alert constraint; the dump is layout noise, not a policy to fight on the glass host. Do **not** disable the confirmationDialog. Do **not** set `translatesAutoresizingMaskIntoConstraints` on UIKit’s glass host. Do **not** invent a second privacy surface. Security / SSL coordinator alerts use the same present hook. Gates: `testPrivacyClearConfirmWaitsUntilSleepTimerDialogDismissed`, `testCoordinatorAlertMustWaitForGlassPopoverHost`.
...
**SeeAlso:** ``RadioPlayerCoordinator/beginObservingVisualStateForChrome()``, ``RadioPlayerCoordinator/handleStatusChange(_:reasonKey:)``, ``RadioPlayerCoordinator/paintBlockedSwitchChromeFromSSOTIfIntentStillNotActive()``, ``RadioPlayerChromeVisualResolver``, ``SleepTimerPrivacyClearPresentation``, ``CoordinatorAlertPresentationSettle``, ``ViewController/presentCoordinatorAlertAfterOutgoingPresentationSettles(_:)``, ``SharedPlayerManager/setPlaying()``, ``SharedPlayerManager/makeEventsStreamWithReplay()``, [`docs/Event-Driven-Refactor-Roadmap.md`](Event-Driven-Refactor-Roadmap.md) (main-app chrome consumer), [`docs/Live-Activity-Stacking-and-Media-Surfaces.md`](Live-Activity-Stacking-and-Media-Surfaces.md), `Lutheran RadioTests/RadioPlayerChromeVisualResolverTests.swift`, `Lutheran RadioTests/PrivacyClearConfirmationPresentationTests.swift`.
...
- **LA start policy:** First `.playing` via ``setPlaying()`` (``.startOrUpdate``): owned updates, eligible + unowned may request, ineligible + unowned records pending ensure; background catch-up via ``RadioLiveActivityManager/handleAppWillEnterBackground()``; termination ends LA immediately.
...
- `RadioPlayerCoordinator+StreamSwitch.swift` — paused/blocked switch tails re-sample SSOT via ``paintBlockedSwitchChromeFromSSOTIfIntentStillNotActive()`` (do not overwrite Connecting after a later play); in-process chip language chrome via ``syncLanguageChromeFromChosenStream(_:isAttaching:)`` (no `pendingAction*` / Darwin note; extension drain remains ``handleWidgetSwitchToLanguage``).
- `ViewController+LayoutHosting.swift` — ``presentCoordinatorAlertAfterOutgoingPresentationSettles(_:)`` / ``CoordinatorAlertPresentationSettle`` (privacy-clear / security / SSL `UIAlertController` after sleep-timer `.confirmationDialog` presented chain is empty **and** glass popover hosts have left the scene).
- `PlaybackControlsView.swift` — ``SleepTimerPrivacyClearPresentation`` (destructive privacy row arms pending; `onClearLocalStateTapped` runs only after `isPresented` is false).
