...
- [ ] Add comprehensive tests for event emission order, stream behavior under actor isolation, and late-subscriber scenarios. **Partial progress (2026-07-07):** `stop()` canonical emission order (`visualStateDidChange` → `playbackIntentChanged` → `streamDidStop` → `.persistedWidgetStateDidUpdate` when the write path runs) is protected via ordered subsequence matching on the DEBUG notification seam. Replay prefix-then-live ordering is protected on the four synthesized state events. Replay live-forwarding after the prefix is exercised as a best-effort check only (XCTest host iterator attach races can still drop forwarded yields). Remaining: emission order for `pause`, `fail`, `start`, language switch, metadata clears, and additional late-subscriber scenarios beyond `stop()`.
...
- [ ] Add comprehensive coverage for every `PlayerEvent` case (emission order, actor isolation, additional late-subscriber scenarios). **Partial:** fourth micro-step (below) finalizes replay prefix + `stop()` order only; other transitions and exhaustive late-subscriber paths remain open.
