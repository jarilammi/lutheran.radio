...
- [ ] Add comprehensive tests for event emission order, stream behavior under actor isolation, and late-subscriber scenarios. **Partial progress (2026-07-07):** `stop()` canonical emission order (`visualStateDidChange` → `playbackIntentChanged` → `streamDidStop` → `.persistedWidgetStateDidUpdate` when the write path runs) is protected via ordered subsequence matching on the DEBUG notification seam. `setUserPaused()` order (`visualStateDidChange` → `playbackIntentChanged` → `streamDidPause` → `.persistedWidgetStateDidUpdate`) is protected by `testSetUserPausedEmissionOrderMatchesCanonicalMutationSequence`. `markPlaybackStoppedByStreamFailure(_:)` order (`visualStateDidChange` → `streamDidFail` → `.persistedWidgetStateDidUpdate`, intent unchanged, no `playbackIntentChanged`) is protected by `testStreamFailureEmissionOrderPreservesIntentAndMutationSequence`. Metadata clear (`clearSoftPauseMetadataStashForLanguageChange()` → `.metadataDidUpdate(nil)` only, no playback mutation) is protected by `testMetadataClearEmitsNilWithoutPlaybackMutation`. Replay prefix-then-live ordering is protected on the four synthesized state events. Replay live-forwarding after the prefix is exercised as a best-effort check only (XCTest host iterator attach races can still drop forwarded yields). Remaining: emission order for `start`, full language-switch paths, and additional late-subscriber scenarios beyond `stop()` / pause / failure.
...
- [ ] Add comprehensive coverage for every `PlayerEvent` case (emission order, actor isolation, additional late-subscriber scenarios). **Partial:** fourth micro-step finalizes replay prefix + `stop()` order; fifth micro-step adds `markPlaybackStoppedByStreamFailure` order + intent-preservation contract; sixth micro-step adds canonical `setUserPaused()` order; seventh micro-step (below) adds metadata clear (`metadataDidUpdate(nil)`). Remaining: `start`, full language-switch paths, and additional late-subscriber paths.
