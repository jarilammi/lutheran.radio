...
**Partial-acceptance dual-axis heal:** ActivityKit can advance **one** ContentState axis while the other still lags (device: destination language accepted with Connecting ``.prePlay`` while audio is already playing — “partial German refresh”; or playing visual accepted with prior language). ``handleActivityContentUpdate`` / ``applySystemContentUpdateHeal(systemContent:)`` apply ``contentUpdateAxisHealPolicy`` against actor SSOT: clear quiet, cancel post-settled retries, and reset ``consecutiveStalledContentPushes`` **only for converged axes** (full non-stalled match via ``shouldResetStalledContentStreak``). Language-only progress **must not** cancel playing post-settled or wipe stall bookkeeping; it **re-arms** playing soft ensure / delayed post-settled retries (``shouldFollowThroughPlayingEnsure`` / ``shouldRearmPlayingEnsureAfterPartialLanguageAcceptance``). Symmetric for visual-only progress with residual language lag. Soft-ensure inter-attempt spacing uses ``softEnsureInterAttemptDelayMilliseconds`` — short while request-eligible (unlock heal), longer while ineligible (``authoritativeContentEnsureIneligibleInterAttemptDelaysMilliseconds``) without raising attempt counts. Does **not** end the only interactive LA while request-ineligible; does **not** invent `.playing` during hold/connect. Unlock / foreground owned-surface ensure remains the presentable safety rail when continuous-lock ActivityKit never applies.
