...
**Device-verified residual (mechanism status, not a reopen of write-side language SSOT):** On real hardware, after a **widget-driven playing stream switch** while interactive request stays **ineligible** (lock / inactive), soft language ensure + post-settled delayed retries may **exhaust without** system-held `content.state.currentLanguage` accepting the destination — audio and home snapshot already show the new language; lock flag/name can lag for the remainder of that inactive stretch. **Presentable-window heal works:** become-active / foreground owned-surface ensure (soft language + playing ensure; eligible recreation only if soft still fails) converges owned language and often owned visual on the **same** interactive activity without a visibility-class end+request hole. Some first switches under lock still accept language immediately; later switches while visual is already lagging Connecting are the harder case. Do **not** treat “soft ensure logs ran” as proof of rendered flag honesty while inactive. Do **not** reverse deferred recreation to “fix” lag while ineligible. Home-widget language honesty is independent and remains strong.
