...
**Interactive recreation after stalled ActivityKit updates:** Soft retries and owned-language suppress cannot repair an ActivityKit surface that never applies `Activity.update` (device freezes: system-held language stuck on a prior stream, or visual stuck on `.userPaused` after soft resume while audio/widgets advance). After each real update, ``isStalledLiveActivityContentPush(candidate:accepted:)`` compares the submitted candidate to re-read `content.state`. A bounded streak (``stalledContentPushRecreationThreshold``) with recreation budget remaining (``maxInteractiveContentRecreations``) triggers ``recreateInteractiveLiveActivityAfterStalledContent()``: ``endActivityAsync(dismissalPolicy: .immediate)`` then ``startActivity()`` so the replacement card is seeded from ``liveActivityLanguageCodeForContentPush()`` + current visual (Connecting honesty preserved during stream-switch hold). Nested recreation during end+start is suppressed; matching chrome or `contentUpdates` clears the streak and recreation count.

Protected by ``RadioLiveActivityManagerTests`` (owned-language suppress gate, language ensure decision, post-update suppress memory, stalled chrome + recreation decision, playing ensure for pause/Connecting, optimistic toggle + stream-switch alignment), ``WidgetIntentContractExtensionTests`` (LA switch language mirror + Connecting toggle mirror), ``WidgetSurfaceTests`` (content replace, stream-switch destination, second-tap plan).
