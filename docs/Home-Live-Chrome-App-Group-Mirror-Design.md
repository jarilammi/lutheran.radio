...
|--------|-------------|----------|-------|
| Home/Control toggle → pause | `.userPaused` | current | Mirror + session optimistic + pending. Instant-feedback language is this **current** stream (``executeOptimisticToggle(plan:language:)`` / ``languageForInstantFeedbackWrite(_:)`` / ``languageForLiveActivityOrWidgetOptimistic()``). The writer coerces a disagreeing caller language (empty extension RAM + locale fallback) to settled chrome. ``loadSharedState()`` still ignores a leftover disagreeing key. |
