...
|--------|-------------|----------|-------|
| Home/Control toggle → pause | `.userPaused` | current | Mirror + session optimistic + pending. Instant-feedback language is this **current** stream (``executeOptimisticToggle(plan:language:)`` / ``languageForInstantFeedbackWrite(_:)`` / ``settledLanguageForInstantFeedback()``). The writer coerces a disagreeing caller language (empty extension RAM + locale fallback, **or** leftover session vs strictly fresher chrome) to the freshness-settled code. ``loadSharedState()`` still ignores a leftover disagreeing key. |
