...
| `isInstantFeedback` / `instantFeedbackTime` / `instantFeedbackLanguage` | Keep for short-lived optimistic **switch** language on `loadSharedState` paths (15 s). Play/pause **proposal** (``languageForPlayPauseOptimisticWrite(resolutionLanguage:)`` / ``languageForLiveActivityOrWidgetOptimistic()``) uses ``settledLanguageForInstantFeedback()`` — session vs ``homeWidgetLiveChrome`` by the same freshness rule as ``resolveHomeWidgetChromeFields`` (chrome wins only when `updatedAt` is strictly newer; ties stay session). A leftover session code is not proposed when chrome is strictly fresher. ``writeInstantFeedback(language:)`` still persists that freshness-settled code when the caller disagrees (empty-RAM locale and leftover-session defense in depth). Play/pause must not plant a leftover session, locale, or lagging Live Activity language. ``loadSharedState()`` still ignores a disagreeing instant-feedback language. **Not** required for Provider visual once the live-chrome mirror exists. Optional later fold (operational residual), not this design’s scope |
...
- Concurrent identical sticky-pause **or** ``.playing`` ``refreshIfNeeded`` calls coalesce via ``shouldCoalesceInFlightIdenticalRefresh`` (visual + language + error + paint epoch) while the first wake is in flight. ``lastKnownState`` stays booked after a real wake. LastKnown identity skip still excludes ``.playing`` so later metadata can adaptive-debounce. The first paint-epoch exception after ``liveChromeIdentitySkipWake`` still executes once. Dual-path event + teardown architecture is unchanged. Scheduler labels are not lock-screen paint and are not rendered home chrome.
...
**Optional rows (2026-08-15):** force-quit residual **passed** on physical device — after process death without `applicationWillTerminate` and past the 60 s `lastUpdateTime` window, home painted passive `tap_to_open` (not prior-process playing); user-initiated open auto-played and live chrome tracked the **new** session. Paused stream switch and remove/re-add widgets were **not** measured. Required §10.3 sequence was **not** re-run.
...
| **Interactive LIVE + open-host** | Entry heal, paint epoch/signature + Darwin wake, direction-bound home play/pause, no thrashy root EntryView `.id` (§6.6) | **Shipped** (2026-08-09) — hop removed earlier same week; intent-miss host open closed on eyes-on |
...
**Optional force-quit residual (2026-08-15):** **Passed** on physical device. After process death without `applicationWillTerminate` and past the 60 s liveness window, home was passive `tap_to_open` — not resurrected prior-process playing. User-initiated main open auto-played; ``homeWidgetLiveChrome`` tracked that session. Paused switch and remove/re-add widgets remain unmeasured. Required §10.3 was not re-run.
...
| Optional §10.3 force-quit residual | **Passed** | 2026-08-15 | Physical device; process death without `applicationWillTerminate`. After the 60 s `lastUpdateTime` window, home was passive `tap_to_open` — not prior-process playing. User-initiated open auto-played; live chrome tracked the new session. Paused switch and remove/re-add widgets not measured. Required §10.3 not re-run. |
...
| Interactive LIVE heal + intent-miss host open closed | **Shipped** | 2026-08-09 | Providers: ``resolveFromSnapshot`` only (``resolveWithActorHygiene`` removed). Interactive entry: always-heal via ``WidgetInteractivePaintHeal``; suite paint epoch/signature + Darwin/local paint-advanced wake as **body deps only** (no root EntryView `.id` thrash — that dropped AppIntent hit targets → host open without pause SSOT). Direction-bound ``WidgetPlayRadioIntent`` / ``WidgetPauseRadioIntent`` (``openAppWhenRun = false``); stable Button identity + expanded hit target. Identity-skip wake kept for definitive visuals without sticky JSON rewrite or dual ``reloadAllTimelines``. Eyes-on: background home pause/play/switch mailbox path while main stays backgrounded. |
