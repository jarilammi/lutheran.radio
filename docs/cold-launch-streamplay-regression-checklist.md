...
6. **Tuning coordination** — `Tuning sound finished playing, success: true` then `Tuning sound wait completed` before attach. Factory-reset Now Playing phase 2 may still be deactivating when presentable cold launch starts; first special-tuning clip / ``play()`` ``configureAudioSessionAsync()`` waits for that deactivate. Overlapping `setCategory` with that deactivate returns SessionCore OSStatus -50. Engine construction does not activate the session.
