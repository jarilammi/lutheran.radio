...
2. `recreatePlayerItem()` — single-flight via `recreateInFlight` on MainActor; always rebuilds via `makeSecuredPlayerItem` (resource loader + DNSSEC/cert path); rebinds `setupPlaybackObservers` + `addObservers`; bails during teardown; discards when `playbackAttachGeneration` advanced; audible restart uses ``shouldAllowAudiblePlaybackKick(startedAt:)`` with the entry snapshot (not ``isCurrentlyAttemptingPlayback``); restarts loading-grace clock on success.
3. `addObservers()` — item `status` / buffer KVO; captures ``playbackAttachGeneration`` at bind and passes that snapshot into ``applyLiveAttachAudibleKickIfReady(itemIsReadyToPlay:isPlaybackLikelyToKeepUp:startedAt:)`` (never fire-time generation); buffer-empty with `AVFoundationErrorDomain` and early-window stalls call secured recreate under intent + budget + loading-grace guards (grace + debounce for pre-stable stalls; longer post-stable stall delay).
...
12. **Explicit-pause switch block** — Widget pause then switch: `[Widget Switch] Blocked — userPaused, no auto-resume`. Blocked-path chrome re-samples intent/visual SSOT after awaits (`paintBlockedSwitchChromeFromSSOTIfIntentStillNotActive`); a later explicit play that already painted Connecting is not overwritten with `.userPaused` (DEBUG skip: intent already active, leaving chrome to SSOT). Intent stays `.userPaused` unless that later play already made it active — the blocked branch never auto-resumes.
...
14. **Widget switch delivery (extension host)** — When the widget extension hosts `SwitchStreamIntent`, it must reach the main app via App Group + Darwin before §6.10 handler lines: `Found pending action: switch` with non-nil `Pending language: {code}`; then `Executing widget switch action to language:`. If pause/play Darwin works but switch produces zero pending-action logs **and the main app was not hosting the intent**, treat as extension routing regression (`isRunningInWidget`, `signalWidgetSwitchAction`), not ICY or stream failure.
...
17. **Presentable in-process chip switch** — When the main app hosts `LiveActivitySwitchStreamIntent` / `SwitchStreamIntent` (`LiveActivityIntent` / `AudioPlaybackIntent`), engine switch must **not** write pending + Darwin (zero `Found pending action: switch` is expected). In-app flags, needle, and decorative background must follow the destination stream (``RadioPlayerCoordinator/syncLanguageChromeFromChosenStream``; DEBUG `Synced language chrome from chosen stream` / `MAIN APP: Updated UserDefaults language`). Do not treat a green XCTest suite that only drains Darwin as proof the presentable app followed the chips.
