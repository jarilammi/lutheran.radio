...
- [ ] **Foreground language ensure with existing interactive activity:** When ownership is non-nil but owned `content.state.currentLanguage` ≠ destination after become-active, run bounded ``ensureAuthoritativeLanguageContentIfNeeded`` (and visual peer). Eligible-only recreation only if soft ensure still fails in foreground. **Do not** end the only LA while request is ineligible. Canonical: [`docs/Live-Activity-Stacking-and-Media-Surfaces.md`](Live-Activity-Stacking-and-Media-Surfaces.md).
...
5. ~~**Live Activity language chrome SSOT**~~ — **Closed (2026-07-19):** `ContentState.currentLanguage` + durable language mirror + chrome from `context.state`; home-widget write suppression unchanged. Deferred-recreation eligibility + soft ensures closed later; optional Tier 1 follow-up for foreground language ensure with existing activity when ContentState trails destination (see Tier 1 open item).
6. Tier 1 optional `WidgetDisplayProjection` bundle (only if future call-site churn warrants it).
7. Optional later: foreground language ensure with existing interactive activity (and eligible-only recreation after soft path only) when system-held ContentState trails destination under deferred recreation.
8. Optional later: XCUITest widget intents / manual QA matrix (high device cost).
9. Optional later: further membership-exception reduction for ``WidgetRefreshManager`` only if coupling can be broken without a circular WidgetSurface ↔ SharedPlayerManager dependency.
