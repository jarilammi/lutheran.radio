...
**ContentState convergence while request-ineligible:** Write-side language stamps, owned-language suppress, soft language ensure, and **deferred** interactive recreation when request is ineligible are shipped. Soft language ensure is bounded: after ``authoritativeLanguageContentEnsureMaxAttempts`` while request is ineligible, language ensure enters **quiet pending** for that destination so status-driven media-surface refreshes do not thrash ActivityKit. Language-only re-pushes defer while quiet; visual mutations still push. Re-arm on destination language change, request eligibility, become-active / foreground (clears quiet), or system `contentUpdates`. While the process remains lock- or background-driven, system-held `content.state.currentLanguage` (and sometimes visual) may still trail destination candidates until ActivityKit accepts an update; audio and the interactive card remain. End+request does **not** run while ineligible (surface continuity). On become-active / foreground with ownership non-nil, ``ensureAuthoritativeContentOnForegroundIfNeeded()`` clears language quiet then runs bounded soft language/playing ensure and may recreate only when request is eligible after soft ensure still fails. Do not re-open eligibility, soft-resume playing ensure, or language-SSOT work as if those mechanisms failed. Canonical mechanism detail: [`docs/Live-Activity-Stacking-and-Media-Surfaces.md`](Live-Activity-Stacking-and-Media-Surfaces.md) (deferred recreation + language ensure quiet pending + foreground owned-surface ensure).

**SeeAlso:** ``RadioLiveActivityManager/updateCurrentActivity()``, ``RadioLiveActivityManager/shouldSuppressLiveActivityContentPush(lastPushed:candidate:ownedContentLanguage:)``, ``RadioLiveActivityManager/ensureAuthoritativeLanguageContentIfNeeded()``, ``RadioLiveActivityManager/shouldRunLanguageContentEnsureSoftPushes(needsLanguageEnsure:destinationLanguage:quietPendingDestination:isRequestEligible:)``, ``RadioLiveActivityManager/ensureAuthoritativeContentOnForegroundIfNeeded()``, ``RadioLiveActivityManager/recordOptimisticStreamSwitchContent(language:visualState:)``, ``SharedPlayerManager/liveActivityLanguageCodeForContentPush()``, ``SharedPlayerManager/mainAppLiveActivityLanguageCode()``, ``SharedPlayerManager/resetToPrePlayForNewStream(preserveActiveSleepTimer:connectingLanguageCode:)``, ``SharedPlayerManager/persistLiveActivityLanguageMirror(_:)``, ``SharedPlayerManager/languageForLiveActivityOrWidgetOptimistic()``, ``WidgetIntentExecution/performLiveActivityToggle()``, ``WidgetIntentExecution/pushOptimisticLiveActivityStreamSwitchContent(languageCode:visualState:)``, ``ContentState/replacingStreamSwitchDestination(language:visualState:clearStreamMetadata:)``, ``WidgetIntentCoordinators/optimisticLiveActivityVisualForStreamSwitch(from:)``, ``displayFlag(for:)``, [`docs/Widget-Presentation-Dataflow.md`](Widget-Presentation-Dataflow.md), [`docs/Live-Activity-Stacking-and-Media-Surfaces.md`](Live-Activity-Stacking-and-Media-Surfaces.md), [`docs/Event-Driven-Refactor-Roadmap.md`](Event-Driven-Refactor-Roadmap.md) (Live Activity language chrome on ContentState shipped).
