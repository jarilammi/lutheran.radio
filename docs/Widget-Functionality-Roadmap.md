...
- **`WidgetRefreshManager`** debouncing/coalescing coordinator: privacy gate (`hasActiveLutheranWidgets`), teardown gate (`isSessionTeardownInProgress`), adaptive debounce, `.prePlay`→`.playing` coalesce, `immediate: true` for sticky pause/lock and `.cleared` (Connecting `.prePlay` is deferred so soft-resume playing can supersede).
...
**ContentState convergence while request-ineligible:** Write-side language stamps, owned-language/visual suppress, soft language + playing ensure, and **deferred** interactive recreation when request is ineligible are shipped. Soft language ensure is bounded: after ``authoritativeLanguageContentEnsureMaxAttempts`` while request is ineligible, language ensure enters **quiet pending** for that destination so status-driven media-surface refreshes do not thrash ActivityKit. Language-only re-pushes defer while quiet; visual mutations still push. Soft playing ensure is bounded the same way: after ``authoritativePlayingContentEnsureMaxAttempts`` while request is ineligible, ``playingEnsureQuietPending`` stops status-driven thrash; visual-only `.playing` repair re-pushes defer while quiet; pause and language mutations still push. Re-arm language quiet on destination change, eligibility, become-active / foreground, or `contentUpdates`; re-arm playing quiet on authoritative play mutation (``rearmPlayingEnsureQuietPending``), optimistic toggle / stream-switch, eligibility, become-active / foreground, or `contentUpdates`. ``setPlaying`` sequences language ensure then re-arm + playing ensure. Soft-ensure thrash protection collapses concurrent soft-push loops per axis, records deferred recreation pending **once** per freeze while ineligible, and rate-limits identical stall diagnostics. While the process remains lock- or background-driven, system-held `content.state` may still trail destination candidates until ActivityKit accepts an update; audio and the interactive card remain. End+request does **not** run while ineligible (surface continuity). On become-active / foreground with ownership non-nil, ``ensureAuthoritativeContentOnForegroundIfNeeded()`` clears both quiet flags then runs bounded soft language/playing ensure and may recreate only when request is eligible after soft ensure still fails. Do not re-open eligibility, soft-resume playing ensure, or language-SSOT work as if those mechanisms failed. Canonical mechanism detail: [`docs/Live-Activity-Stacking-and-Media-Surfaces.md`](Live-Activity-Stacking-and-Media-Surfaces.md) (deferred recreation + language/playing ensure quiet pending + thrash collapse + foreground owned-surface ensure).

**SeeAlso:** ``RadioLiveActivityManager/updateCurrentActivity()``, ``RadioLiveActivityManager/shouldSuppressLiveActivityContentPush(lastPushed:candidate:ownedContentLanguage:)``, ``RadioLiveActivityManager/ensureAuthoritativeLanguageContentIfNeeded()``, ``RadioLiveActivityManager/ensureAuthoritativePlayingContentIfNeeded()``, ``RadioLiveActivityManager/rearmPlayingEnsureQuietPending()``, ``RadioLiveActivityManager/shouldRunLanguageContentEnsureSoftPushes(needsLanguageEnsure:destinationLanguage:quietPendingDestination:isRequestEligible:)``, ``RadioLiveActivityManager/shouldRunPlayingContentEnsureSoftPushes(needsPlayingEnsure:quietPending:isRequestEligible:)``, ``RadioLiveActivityManager/ensureAuthoritativeContentOnForegroundIfNeeded()``, ``RadioLiveActivityManager/recordOptimisticStreamSwitchContent(language:visualState:)``, ``SharedPlayerManager/liveActivityLanguageCodeForContentPush()``, ``SharedPlayerManager/mainAppLiveActivityLanguageCode()``, ``SharedPlayerManager/resetToPrePlayForNewStream(preserveActiveSleepTimer:connectingLanguageCode:)``, ``SharedPlayerManager/persistLiveActivityLanguageMirror(_:)``, ``SharedPlayerManager/languageForLiveActivityOrWidgetOptimistic()``, ``WidgetIntentExecution/performLiveActivityToggle()``, ``WidgetIntentExecution/pushOptimisticLiveActivityStreamSwitchContent(languageCode:visualState:)``, ``ContentState/replacingStreamSwitchDestination(language:visualState:clearStreamMetadata:)``, ``WidgetIntentCoordinators/optimisticLiveActivityVisualForStreamSwitch(from:)``, ``displayFlag(for:)``, [`docs/Widget-Presentation-Dataflow.md`](Widget-Presentation-Dataflow.md), [`docs/Live-Activity-Stacking-and-Media-Surfaces.md`](Live-Activity-Stacking-and-Media-Surfaces.md), [`docs/Event-Driven-Refactor-Roadmap.md`](Event-Driven-Refactor-Roadmap.md) (Live Activity language chrome on ContentState shipped).
...
- [x] **Settled language acceptance after hold clear (2026-07-29):** ``pushSettledLanguageAcceptanceContentIfNeeded()`` fires one high-signal dual-axis ContentState push after stream-switch hold clears (``setPlaying`` / soft-resume no-op), bypassing language quiet once per destination while request is ineligible so attach-storm quiet cannot freeze flag chrome for the whole lock stretch. Consume-once; re-open on destination change, eligibility, become-active, or `contentUpdates`. Does **not** invent `.playing` during hold; does **not** end while ineligible. Gate: `testSettledLanguageAcceptancePushAfterHoldClearWhileQuiet`. Canonical: [`docs/Live-Activity-Stacking-and-Media-Surfaces.md`](Live-Activity-Stacking-and-Media-Surfaces.md).
- [x] **Settled playing acceptance after hold clear (2026-07-29):** ``pushSettledPlayingAcceptanceContentIfNeeded()`` fires one high-signal dual-axis ContentState push after hold/connect clear when the actor is authoritative `.playing` and owned visual still lags (``.prePlay`` / ``.userPaused``), bypassing playing quiet once while request is ineligible so attach-storm / soft-resume quiet cannot freeze Connecting chrome for the whole lock stretch. Consume-once; re-open on optimistic toggle / stream-switch, eligibility, become-active, or `contentUpdates`. ``setPlaying`` / soft-resume sequence settled language then settled playing then soft playing ensure (no blind re-arm). Does **not** invent `.playing` during hold/connect; does **not** end while ineligible. Gate: `testSettledPlayingAcceptancePushAfterHoldClearWhileQuiet`. Canonical: [`docs/Live-Activity-Stacking-and-Media-Surfaces.md`](Live-Activity-Stacking-and-Media-Surfaces.md).
...
- **2026-07-29:** **Settled language acceptance after hold clear** — Soft language ensure can exhaust during stream-switch Connecting and enter quiet pending, which then defers language-only status re-pushes for the rest of a lock stretch. ``pushSettledLanguageAcceptanceContentIfNeeded()`` runs after hold clear on ``setPlaying`` / soft-resume no-op: one dual-axis ``updateCurrentActivity`` with quiet bypass once per destination while ineligible; re-enters quiet if still lagging; consume clears on destination change, eligibility, become-active, or `contentUpdates`. Does **not** invent `.playing` during hold; does **not** end while ineligible. Gate: `testSettledLanguageAcceptancePushAfterHoldClearWhileQuiet`. Canonical: [`docs/Live-Activity-Stacking-and-Media-Surfaces.md`](Live-Activity-Stacking-and-Media-Surfaces.md). No security surface change; privacy write suppression and home-widget write gating unchanged.
...
- **2026-07-23:** **Retired App Group orphans** — stop writing `preferredVolume` (system volume SSOT via `MPVolumeView`; engine gain unity after attach) and `lastUserPauseTime` (in-actor ``lastUserPauseTimestamp`` / ``wasRecentlyUserPaused(within:)`` only; zero disk readers). Both keys purged by ``clearPersistedVisualStateKeysFromDisk()``. SSOT table updated. Gate: extended `testColdLaunchFactoryResetClearsDiskVisualStateAndReturnsPrePlay`. Canonical: `SharedPlayerManager.swift` App Group table. No security surface change.
