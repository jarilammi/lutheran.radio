...
- [x] **Foreground language ensure with existing interactive activity (2026-07-29):** When ownership is non-nil but owned `content.state.currentLanguage` ≠ destination (or visual peer lags) after become-active / foreground, ``ensureAuthoritativeContentOnForegroundIfNeeded()`` runs bounded ``ensureAuthoritativeLanguageContentIfNeeded`` + ``ensureAuthoritativePlayingContentIfNeeded``. Eligible-only recreation only if soft ensure still fails in foreground. **Does not** end the only LA while request is ineligible. Wired through ``ensureInteractiveLiveActivityIfNeeded()`` (SceneDelegate become-active + ``handleAppDidEnterForeground``). Gates: `testForegroundOwnedSurfaceSoftEnsurePolicy`, `testForegroundRecreationOnlyAfterSoftEnsureFailsAndRequestEligible`. Canonical: [`docs/Live-Activity-Stacking-and-Media-Surfaces.md`](Live-Activity-Stacking-and-Media-Surfaces.md).
...
- **2026-07-29:** **Foreground language ensure with existing interactive Live Activity** — When ownership is non-nil after become-active / foreground, ``ensureAuthoritativeContentOnForegroundIfNeeded()`` runs bounded soft language + playing ensure; eligible-only recreation only if soft ensure still fails (never while request is ineligible). Language ensure retries up to ``authoritativeLanguageContentEnsureMaxAttempts``. Wired through ``ensureInteractiveLiveActivityIfNeeded()`` so SceneDelegate become-active and ``handleAppDidEnterForeground`` share one path. Gates: `testForegroundOwnedSurfaceSoftEnsurePolicy`, `testForegroundRecreationOnlyAfterSoftEnsureFailsAndRequestEligible`. Canonical: [`docs/Live-Activity-Stacking-and-Media-Surfaces.md`](Live-Activity-Stacking-and-Media-Surfaces.md). No security surface change; privacy write suppression and home-widget write gating unchanged.
