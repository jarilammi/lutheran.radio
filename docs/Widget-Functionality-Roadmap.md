...
- [x] **Playing ensure quiet pending while request ineligible (2026-07-29):** After ``authoritativePlayingContentEnsureMaxAttempts`` without owned `.playing` acceptance while interactive request is ineligible, soft playing ensure enters quiet pending; visual-only `.playing` repair re-pushes defer; pause and language mutations still push. Re-arm on authoritative play mutation (``rearmPlayingEnsureQuietPending`` from ``setPlaying`` / soft-resume), optimistic toggle / stream-switch, eligibility, become-active (clears quiet), or `contentUpdates`. ``setPlaying`` sequences language ensure then re-arm + playing ensure. Does **not** invent `.playing` during hold; does **not** end while ineligible. Gates: `testPlayingEnsureQuietPendingAfterMaxAttemptsWhileIneligible`, `testPlayingOnlyStatusPushDefersWhileQuietPending`. Canonical: [`docs/Live-Activity-Stacking-and-Media-Surfaces.md`](Live-Activity-Stacking-and-Media-Surfaces.md).
