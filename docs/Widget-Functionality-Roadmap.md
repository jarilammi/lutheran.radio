...
| Language chrome (flag + name + alt-stream “current”) | Snapshot `currentLanguage` via Provider assembly / ``preferredWidgetLanguage()`` (home/Control; App Group + session hygiene) | N/A (not shown) | **Open gap:** views still call ``preferredWidgetLanguage()`` at render time; language is **not** on `ContentState` (see Open Issues) |
...
### Live Activity language chrome SSOT (open — same class as LA toggle visual fix)

**Status:** Open (highest-priority functional backlog for Live Activity presentation)

**Observable defect**

Lock Screen and Dynamic Island language chrome (flag emoji + language name, plus which code is treated as “current” for the alt-stream row) shows **English** while the main app is playing a non-English stream (for example Finnish, Estonian, or German). Program title / speaker from ICY can still be correct on the same surface because they flow through `ContentState.streamMetadata`. System Now Playing artwork is unrelated (generic `radio-placeholder`, not a language flag).

**Why it happens (production architecture, not a race)**

Three permanent constraints combine:

1. **`ContentState` carries only `visualState` + `streamMetadata`.**  
   ``RadioLiveActivityManager/updateCurrentActivity()`` and `startActivity` build  
   `LutheranRadioLiveActivityAttributes.ContentState(visualState:streamMetadata:)` only.  
   Main-app stream language never rides the ActivityKit push that already updates visual + metadata.

2. **LA views re-derive language at render time outside the pushed payload.**  
   `LockScreenLiveActivityView` and the Dynamic Island expanded leading region call  
   ``SharedPlayerManager/preferredWidgetLanguage()`` (and use that for  
   ``displayFlag(for:)`` / language name / alt-stream “current”). That is extension-host  
   resolution, not `context.state`.

3. **``preferredWidgetLanguage()`` cannot see main-app language under LA-only sessions.**  
   - ``loadPersistedWidgetState()`` is **memory-only** (`inMemorySessionWidgetSnapshot`). Main-app memory is not the extension process; extension restarts see `nil`.  
   - When ``hasActiveWidgets`` is **false** (no home-screen / Control widget configs — privacy write suppression), authoritative App Group snapshot writes and many language keys are suppressed. Live Activity may still run.  
   - Fallthrough is App Group legacy `currentLanguage` or hard default **`"en"`**.

Same class of defect as lock-screen **toggle visual** planning before ContentState + durable mirror: extension-local / privacy-gated reads diverge from the in-memory main-app stream SSOT. Visual planning was fixed; language chrome was not.

**Corroborating intent path**

LA play/pause optimistic paths (`handleWidgetPlay` / `handleWidgetStop` / signal helpers) call  
`writeInstantFeedback(language: preferredWidgetLanguage())`. When resolution defaults to `"en"`, instant-feedback language is stamped English even though the engine stream is `fi` / `et` / `de`. Explicit ``LiveActivitySwitchStreamIntent`` / ``signalWidgetSwitchAction`` paths pass a concrete language code and therefore write the correct optimistic language — they do not repair chrome that still reads `preferredWidgetLanguage()` on every ActivityKit re-render after process recycle.

**Required end state (implementation contract)**

| Concern | Required SSOT | Writer | Reader |
|---------|---------------|--------|--------|
| LA language chrome (flag, name, alt “current”) | `ContentState.currentLanguage` (or equivalent named field) | Main-app ``RadioLiveActivityManager`` on every start/update, from the same main-app language source used for stream attach (actor / coordinator language — **not** privacy-gated ``preferredWidgetLanguage()`` under no-widgets) | `LockScreenLiveActivityView`, Dynamic Island language chrome — **only** `context.state` (pre-hoist once like status/control/metadata) |
| Optional durable mirror | App Group key analogous to ``liveActivityToggleVisualState`` | Same push sites as visual mirror; **not** gated by ``hasActiveWidgets`` | Intent/optimistic paths that need language when ActivityKit activities are unavailable |
| Play/pause optimistic language | ContentState language and/or durable mirror | ``WidgetIntentExecution/performLiveActivityToggle()`` and related signal helpers | Instant feedback + any extension snapshot write — **never** bare ``preferredWidgetLanguage()`` when that defaults to `"en"` with no snapshot |

**Explicit non-goals**

- Do **not** re-enable full home-widget `PersistedWidgetState` / `performActualSave` writes solely to feed LA language when ``hasActiveWidgets`` is false (privacy regression).
- Do **not** put security, DNS, or certificate logic in WidgetSurface or LA attributes.
- Do **not** change Now Playing artwork policy unless product asks; this issue is LA language chrome + optimistic language for LA intents.

**Suggested micro-steps (small, reviewable, any order that preserves green gates)**

1. Extend ``LutheranRadioLiveActivityAttributes.ContentState`` with `currentLanguage: String` (Codable/Hashable/Sendable). Defaulting strategy for older activities must be explicit and tested.
2. Thread language through ``RadioLiveActivityManager`` candidate construction (`startActivity`, `updateCurrentActivity`, final end content if language is still shown). Source: main-app current stream language (same as `Setting stream to:` / UserDefaults language update path), with a single documented fallback.
3. Hoist `context.state.currentLanguage` once in `LockScreenLiveActivityView` and the outer `dynamicIsland` closure; replace all LA chrome uses of ``preferredWidgetLanguage()`` for flag/name/alt-current. Keep ``displayFlag(for:)`` / pure name helpers — only the **code input** changes.
4. Align ``lastPushedContent`` equality so language changes force an ActivityKit update (dedup must not suppress language-only transitions).
5. Fix LA play/pause optimistic language to prefer ContentState / durable language mirror over ``preferredWidgetLanguage()``.
6. Optional: durable App Group language mirror (write on every LA content push; clear on LA end, termination, factory reset — mirror visual-mirror lifecycle).
7. Tests (main-app host + extension profile as appropriate): ContentState carries language across stream switches with ``hasActiveWidgets == false``; chrome assembly uses state language; toggle optimistic path does not rewrite feedback language to `"en"` when state/mirror is non-English; factory reset / end clears any durable language mirror.

**Files to touch (expected)**

- `WidgetSurface/LutheranRadioLiveActivityAttributes.swift`
- `Lutheran Radio/RadioLiveActivityManager.swift`
- `LutheranRadioWidget/LutheranRadioWidgetLiveActivity.swift`
- `Lutheran Radio/SharedPlayerManager.swift` / `WidgetDisplayModels.swift` (optimistic + optional mirror only)
- Tests under `Lutheran RadioTests/`, `LutheranRadioWidgetTests/`, `WidgetSurfaceTests/` as contracts require
- This roadmap + [`docs/Widget-Presentation-Dataflow.md`](Widget-Presentation-Dataflow.md) presentation contract (ContentState field list)

**SeeAlso:** ``RadioLiveActivityManager/updateCurrentActivity()``, ``SharedPlayerManager/preferredWidgetLanguage()``, ``SharedPlayerManager/persistLiveActivityToggleVisualStateMirror(_:)``, ``WidgetIntentExecution/performLiveActivityToggle()``, ``WidgetIntentExecution/performLiveActivityStreamSwitch(languageCode:)``, ``displayFlag(for:)``, Open Issue “Lock-screen Live Activity toggle planning” (fixed pattern to mirror), [`docs/Widget-Presentation-Dataflow.md`](Widget-Presentation-Dataflow.md), [`docs/Event-Driven-Refactor-Roadmap.md`](Event-Driven-Refactor-Roadmap.md) (OI-2 cross-link).
...
- [ ] **Live Activity language on `ContentState` + chrome from `context.state` (open — see Open Issues):** Add `currentLanguage` to ``LutheranRadioLiveActivityAttributes.ContentState``; push from ``RadioLiveActivityManager`` using main-app stream language; Lock Screen / Dynamic Island language chrome (flag, name, alt “current”) read **only** `context.state.currentLanguage` (hoisted once). Stop using ``preferredWidgetLanguage()`` for LA display. Align `lastPushedContent` equality so language-only changes update. Full contract under Open Issues “Live Activity language chrome SSOT”.
- [ ] **LA play/pause optimistic language (paired with ContentState language):** Toggle/signal paths must not stamp instant-feedback language via bare ``preferredWidgetLanguage()`` when ContentState / durable language mirror already holds the active stream code.
- [ ] **Optional durable LA language App Group mirror:** Same lifecycle and non-`hasActiveWidgets` gating as ``liveActivityToggleVisualState``; only if intent hosts still need language without a live ActivityKit activity list.
...
- [x] **Media-transport command serialization (2026-07-18):** ``MediaTransportCommand`` mailbox on ``SharedPlayerManager`` serializes Now Playing / headset play-pause-toggle-stop and main-app Live Activity engine execution (`submitMediaTransportCommand` / `AndWait`). Pause/stop preempt in-flight play; toggle samples `isActivelyPlaying` only after prior verbs commit. Gates: `testMediaTransportDoubleToggleWhilePlayingEndsPlaying`, `testMediaTransportDoubleToggleWhilePausedEndsPaused`, `testMediaTransportPausePreemptsInFlightPlayOnMailbox`, `testMediaTransportInterleavesLiveActivityPauseWithRemoteToggle`. Canonical: [`docs/Live-Activity-Stacking-and-Media-Surfaces.md`](Live-Activity-Stacking-and-Media-Surfaces.md).
...
5. **Live Activity language chrome SSOT** (Open Issue + Tier 1 checkboxes) — **next functional work**: `ContentState.currentLanguage` → LA views from `context.state` → toggle optimistic language → optional durable language mirror → tests under ``hasActiveWidgets == false``. Mirror the fixed LA toggle **visual** multi-source pattern; do not reopen home-widget write suppression.
